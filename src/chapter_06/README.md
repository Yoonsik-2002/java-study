...
class Data {int x;} // 인스턴스 변수 x. Data 클래스의 인스턴스가 생성된 뒤에 사용이 가능하다. 
...
해당 코드를 보면, `change` 메서드의 매개변수가 참조형인 것을 알 수 있다. 인자로는, 인스턴스의 주소값을 저장하고 있는 참조변수<br>
`d`가 전달되는 것을 알 수 있다.<br>

이러한 참조형 매개변수의 경우, 저장하고 있는 인스턴스의 주소값이 복사되어 전달되게 된다. 그렇기 때문에, 해당 메서드의 매개변수<br>
또한, 이러한 인스턴스의 주소값을 복사하여 저장하게 되어, 해당 인스턴스를 가리키고 있는 상태가 된다.<br>

이렇게, 인스턴스를 가리키고 있는 상태에서, 해당 인스턴스의 멤버변수 `x`의 값을 변경하면, 이러한 변경사항은 실질적으로 적용된다.<br>
