...
멤버변수와 같은 이름으로 붙이는 경우가 허다하다.<br>
(생성자 : *'매개변수에 전달된 값으로 멤버변수(인스턴스 변수)의 값을 초기화 하라!'*)
...
위 코드는 생성자의 매개변수의 이름과 값을 전달받아, 초기화 되는 인스턴스 변수의 이름과 똑같이 해 주었지만,<br>
이, `color = color`의 경우, 두 `color`는 매개변수 `color`를 접근하기 때문에, 모두 매개변수로 선언된 지역변수 `color`로<br>
간주된다.<br>

이런경우, 즉, 초기화를 할 인스턴스 변수와 생성자의 매개변수의 이름이 같은 경우 인스턴스 변수 앞에 `this`를 사용하면 된다.<br>

이 `this`는 참조변수로, 인스턴스 자신을 가리킨다. 인스턴스의 참조변수를 통해 해당 인스턴스의 멤버들에 접근할 수 있는 것 처럼<br>
`this`로 인스턴스 변수에 접근이 가능한 것이다!<br>

그러므로, 두 번째 예시코드의 의미는 다음과 같다.<br>
(주석을 통해 작성해 놓았다.)
```java
//[Example_2]
