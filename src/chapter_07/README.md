...
interface 인터페이스이름 {
  ...
  publis abstract 반환형 메서드이름(매개변수 목록);  // 추상 메서드
}
```
<br>

이때, 위 선언문의 형태를 보면 알 수 있듯이, 인터페이스의 경우, 상수이든, 추상 메서드이든 예외없이 **인터페이스의 멤버라면 모두 `public`이 붙는다.** <br>

또, 이러한 인터페이스의 멤버, 상수와 추상 메서드에는 다음과 같은 제약사항이 존재한다.<br>
> ##### 추상 메서드-
...
추상 메서드의 경우, 기본적으로 `abstract`가 붙는데, 인터페이스의 멤버는 모두 `public`이 붙어야 하므로, 인터페이스의 추상 클래스 멤버는 항상 예외없이 `public abstract`인 것이다.<br>

또, 상수의 경우를 설명하면, Java에서 상수의 접근 제어자는 일반적으로 `static final`로 선언되는데, 
