...
인터페이스는 무엇인가? 메서드의 선언부, 즉, 추상 메서드들을 정렬해 놓은 것이라 할 수 있다. 때문에, 아직 구현되지 않은 인터페이스의 추상 메서드라도, 해당 메서드를 호출하는 클래스를 바로 작성하는 것이 가능하다.<br>

이렇게, 메서드의 선언부를 통해, 해당 메서드를 호출하는 클래스를 작성하고 있는 동안, 해당 메서드의 선언부(추상 메서드)를 선언하고 있는 인터페이스를 구현해주는 클래스를 동시에 작성하는 것이 가능하다. 
<br>
