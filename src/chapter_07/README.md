...
### 인터페이스를 사용하여, 두 클래스의 관계를 간접적인 관계로 만들어 주는 경우 `A`-`I`-`B`
...
이제, 클래스`A`와 클래스`B`의 관계는 클래스`A`가 클래스`B`를 직접 사용하여 작성된 `A`-`B`의 직접적인 관계에서, 클래스`A`가 클래스`B`대신 인터페이스`I`를 사용하여 작성됨으로써, `A`-`I`-`인터페이스I를 구현한 클래스(B)`의 간접적인 관계로 바뀌게 되었다.<br> 
