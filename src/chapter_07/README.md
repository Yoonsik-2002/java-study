...
그렇다면, 해당 경우에는 4개의 접근 제어자들 중, 무엇을 사용하는 것이 좋을까? 먼저 첫 번째로 떠오르는 것은 `private`일 것이다. 그리고, `Time`클래스의 자손 클래스를 생성하여 해당 클래스를 확장해야하는 상황이라면, `protected`를 사용해야 할 것이다.<br>
```java
class Time {
  private int hour;
  private int minute;
...
이렇게, 접근 제어자`private`을 이용한 캡슐화를 통하여, 외부에서 `Time`클래스의 멤버에 직접 접근하지 못하게 하였다. 이제 Time클래스 내에서만 해당 멤버변수들에 접근하는 것이 가능하다. 
