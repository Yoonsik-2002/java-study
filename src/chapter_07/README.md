...
특정한 값의 범위가 정해져 있는 `Time`클래스의 멤버변수 `hour`, `minute`, `second`에 접근 제어자 `private`이나 `protected`를 사용하여, 외부에서 함부로 접근하지 못하도록 접근을 제한하고,<br>

해당 멤버변수들을 범위에 맞는 올바른 값으로 변경하고, 읽어낼 수 있는 `public`메서드를 통해 간접적으로 멤버변수의 값을 다룰 수 있게 하는 방법을 사용하면, 멤버변수들을 외부로부터 보호하면서도, 올바른 값으로 변경해 주는 것이 가능하다.<br>
