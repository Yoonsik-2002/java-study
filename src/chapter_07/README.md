...
먼저, `Marine`클래스와 `Tank`클래스는 지상 유닛이지만, `DropShip`클래스는 공중 유닛으로, 서로 이동하는 방법이 다르다는 차이점이 존재한다. 때문에, 이동 기능을 구현하는 `move`메서드의 경우, 실제 기능 구현을 담당하는 메서드의 구현부의 내용이 클래스 간 다를 것이다.<br>

하지만, 해당 클래스들 간, `move`메서드의 선언부는 같기 때문에
