...
`Car`타입의 참조변수 `c`를 매개변수로 하는 위 `doWork` 메서드가 호출될 때, 매개변수로는 어떠한 값이 전달될까? 참조변수 `c`를 통해서는 어떠한 값들이 전달될 수 있을지 그림으로 나타내 보았다.<br>
<br>
...
`if(c instanceof FireEngine)`의 경우를 한번 보자. 참조변수 `c`를 `FireEngine`클래스 타입으로 형 변환이 가능한지 검사해주는 문장이다. 이를 자세히 풀어서 설명하면, 아래와 같은 검사를 해 주는 것이다.<br>
