...
모든 클래스들의 조상클래스이자, 클래스 계층의 root인(최상위인) `Object`클래스. 이러한 이유로, **자바의 모든 클래스들은 이<br>
`Object`클래스에 정의되어 있는 멤버들을 사용하는 것이 가능하다.** <br>

그동안, `toString()`이나, `equals(Object obj)`와 같은 메서드를 정의하지 않고도 사용할 수 있었던 이유는, 해당 메서드들이<br>
`Object`클래스에 정의되어 있기 때문이다.<br>
(자바의 모든 클래스들이 `Object`클래스를 상속받기 때문에, 해당 클래스의 멤버인 `toString()`메서드를 따로 정의하지 않고도,<br>
그대로 사용하는 것이 가능!)<br>
...
기대와는 달리 해당 메서드를 통해 리턴된 값이 전혀 간결하지도, 읽기 쉽지도, 또 이것이 어떤 정보를 제공하는지도 알기가 쉽지<br>
않았다.<br>
