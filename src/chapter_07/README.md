...
자손 클래스는 조상 클래스의 멤버를 상속받아, 이를 확장한 개념이라 할 수 있다. 이러한 자손 클래스 타입의 참조변수로 조상 클래스 타입의 인스턴스를 참조하는 경우, 참조 변수가 사용할 수 있는 멤버의 개수가 실제 참조하고 있는 인스턴스가 가지는 멤버의 개수보다 많아지게 된다.<br>
<br>
