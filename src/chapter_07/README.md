...
위 코드는 `Car`타입의 참조변수 `c`를 매개변수로 하는 `doWork` 메서드이다. 해당 메서드가 호출될 때, 매개변수로는 어떠한 값이 전달될까?<br>

먼저, `Car`클래스의 인스턴스가 전달될 수 있겠고, 조상 클래스 타입의 참조변수가 자손 클래스의 인스턴스를 참조하는 것도 가능하므로, `Car`클래스의 자손 클래스인 `FireEngine`클래스의 인스턴스나 `Ambulance`클래스의 인스턴스가 매개변수로 전달될 수 있을 것이다.<br>

또, 추가적으로 `c`가 아무런 인스턴스를 가리키지 않고 `null`값을 저장하고 있는 경우엔 `null`값이 매개변수로 전달될 수도 있겠다.<br>

이렇게, 메서드 내에서는 정확히 어떠한 인스턴스 혹은 값(`null`)을 매개변수로 전달받은 것인지 알 방법이 없다. 그렇기에, `instanceof`연산자를 활용하여 참조변수 `c`가 참조하고 있는 인스턴스의 타입을 확인하고난 뒤, 참조변수의 형변환을 해 주어야 한다.<br>
<br>

`if(c instanceof FireEngine)`의 경우, `c`가 참조하고 있는 인스턴스가 `Car`클래스의 인스턴스라면, `FireEngine`클래스 타입으로 형 변환하는 것이 가능할까? 아니다. 자손 클래스 타입의 참조변수로 조상 클래스의 인스턴스를 참조하는 꼴이되기 때문에 허용되지 않는다.<br>

그렇다면, `c`가 참조하고 있는 인스턴스가 `Ambulance`클래스의 인스턴스라면, `FireEngine`클래스 타입으로 형 변환하는 것이 가능할까? 아니다. 서로 상속 관계에 있는 클래스 간의 형 변환만이 가능하다. 클래스 간의 형제관계라는 것은 존재하지 않는다.<br>

그럼, `c`가 참조하고 있는 인스턴스가 `FireEngine`클래스의 인스턴스라면 `FireEngine`클래스 타입으로 형 변환한는 것이 가능할까? 가능하다. 형 변환시, 참조변수와 인스턴스의 타입이 일치하게 되는 상황으로, 인스턴스가 가지는 모든 멤버를 사용하는 것이 가능하게 된다.<br>
