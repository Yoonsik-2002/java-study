...
먼저, 아래 코드를 한번 보자. 생성자에 `private`접근 제어자가 붙어 있는 것을 알 수 있다. `private`생성자는 `Singleton`클래스 내에서만 사용이 가능하다. 때문에, 해당 생성자의 호출이 필요한 `Singleton`클래스의 인스턴스의 생성도 `Singleton`클래스 내에서 이루어져야 한다.<br>
...
아래 코드는 `Singleton`클래스 내부에서 생성된 인스턴스를 반환해주는 `getInstance`메서드 이다. 외부에서 `private`생성자를 통해서는 `Singleton`클래스의 인스턴스를 생성할 수 없다.<br>

때문에, 내부에서 생성한 인스턴스를 리턴해주는 메서드 `getInstance`는 `Singleton`클래스의 인스턴스를 생성해야지만 사용이 가능한 인스턴스 멤버가 아닌, `Singleton`클래스의 인스턴스를 생성하지 않아도 사용이 가능한 클래스 멤버여야 한다.<br>
