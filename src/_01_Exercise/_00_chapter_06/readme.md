...
- #### `public static int abs(int a)`
  ...
  ##### 인수가 양수인 경우 - 
  전달받은 인수(메서드가 호출될 때 넘기는 변수 값)가 양수라면, 해당 인수는 그대로 리턴된다.<br>

  ##### 인수가 음수인 경우 - 
  전달받은 인수가 음수라면, 해당 인수값의 반대값(양수)이 리턴된다.<br>

  ##### 주의할 점 - 
  인수가 표현할 수 있는 가장 음의 값인 `integer.MIN_VALUE`의 값과 같다면, 동일한 값(음수)이 그대로 반환된다.<br>
