...
### 📑 Review
- ### 위 코드를 컴파일 하면, 에러가 발생하는 이유에 대해서 정리
  ...
  이렇게 조상 클래스를 포함하는 자손 클래의 생성자에서는 반드시 조상 클래스의 생성자를 첫 줄에서 명시적으로 호출해 주어야 한다. 이때, 자손 클래스에서 조상 클래스의 생성자를 호출하기 위해 사용되는 키워드가 바로 `super`키워드 이다.<br>
