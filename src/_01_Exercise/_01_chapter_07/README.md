...
### 📑 Review
위 코드에서 `attack`메서드의 매개변수에 들어갈 수 있는 값으로는, 다음 것들이 있다.<br>

- 인터페이스 `Movable`을 구현한 클래스의 인스턴스
- 인터페이스 `Movable`을 구현한 클래스의 자손 클래스의 인스턴스
...
### 📑 Review
- ###### 인터페이스와 이를 구현하는 클래스(껍데기와 알맹이의 관계)
  인터페이스란, 추상 메서드들을 나열해 놓은 것으로, 해당 인터페이스를 구현한 클래스의 껍데기와 같은 역할을 한다.<br>

  이러한 인터페이스가 클래스의 껍데기라 불리는 이유는, 클래스가 가져야 할 기능을 정의하지만, 구체적인 구현 내용은 제공하기 않기 때문이라 할 수 있다. 이렇게, 인터페이스는 속이 비어있는 껍데기로 비유하는 것이 가능하다.<br>

  이러한 인터페이스라는 빈 껍데기를 알맹이로써 채워주는 것이 바로, 인터페이스를 구현하는 클래스라 할 수 있다.<br>

  인터페이스가 정의한 클래스가 가져야할 기능들(추상 메서드들)을 구체적인 구현 내용을 통해 구현하여 속을 채워주는 것이 바로, 인터페이스를 구현하는 클래스이다.<br>
<br>

- ###### 인터페이스를 사용하면 얻을 수 있는 장정
  #### 1) 독립적인 프로그래밍이 가능하며, 변경에 유리한, 유연한 코드를 작성하는 것이 가능하다.
  어느 한 클래스가 다른 클래스를 사용하는 경우, 두 클래스가 서로 직접적인 관계를 가진다고 표현하는 것이 가능하다.<br>
  
  이렇게 두 클래스가 서로 직접적인 관계에 있는 경우, 둘 중 어느 한 클래스가 변경되면, 다른 한 클래스도 이에 맞게 변경해 주어야 한다. 즉, 변경에 불리한 유연하지 못한 코드가 되는 것이다.<br>
  <br>

  반면, 인터페이스를 사용하는 경우, 클래스의 선언(인터페이스)와 클래스의 구현(인터페이스를 구현하는 클래스 작성)을 분리하는 것이 가능하다.<br>

  이를 통해 클래스가 서로 직접적인 관계가 아닌, 인터페이스를 사이에 둔 간접적인 관계를 가지게 하여, 한 클래스의 변경이 다른 클래스에게 영향을 미치지 않는 독립적인 프로그래밍과 변경에 유리한, 유연한 코드를 작성하는 것이 가능하게 된다.<br>
  <br>

  #### 2) 개발시간을 단축하는 것이 가능하다.
  일단 인터페이스가 작성되면, 이를 사용하여 프로그램을 작성하는 것이 가능하다.<br>

  그 이유는, 메서드를 호출하는 쪽에서는 메서드의 내용(구현부)와는 관계없이 선언부(어떠한 값을 대입해 주어야 하고, 결과로 어떠한 값을 반환받을 수 있는지)만 알면 되기 때문이다.<br>

  때문에, 클래스 `A`가 인터페이스 `I`를 구현한 클래스 `B`와 인터페이스 `I`를 사이에 두고 서로 간접적인 관계를 가지고 있는 경우, 클래스 `A`는 클래스 `B`가 완성될 때까지 기다릴 필요가 없다.<br>

  클래스 `A`는 클래스 `B`가 구현해 주어야 하는 인터페이스에 나열되어 있는 추상 메서드(선언문)를 이용하여 작성해주면 되고, 동시에 클래스 `B`는 인터페이스 `I`를 구현하여 작성해 주면 되는 것이다.<br>

  이렇게 인터페이스를 사용하면, 해당 작업을 동시에 수행하는 것이 가능하여, 개발 시간을 단축시키는 것이 가능하다.<br>
  <br>

  #### 3) 표준화가 가능하다.
  프로젝트에 사용되는 기본 틀을 인터페이스로 작성하고, 개발자들에게 해당 인터페이스를 구현하여 프로그램을 작성하게 함으로써, 보다 일관적이고 정형화된 프로그램을 개발하는 것이 가능하다.<br>
  <br>

  #### 4) 서로 관련이 없은 클래스들의 관계를 맺어주는 것이 가능하다.<br>
  서로 관계없는 클래스들이 하나의 동일한 인터페이스를 구현하게 함으로써, 새로운 관계를 맺어주는 것이 가능하다.<br>

  이때, 해당 인터페이스가 비어있는 인터페이스여도 가능하다. 비어있는 인터페이스를 구현하게 함으로써, 클래스들의 관계만을 맺어주는 것이다.<br>
